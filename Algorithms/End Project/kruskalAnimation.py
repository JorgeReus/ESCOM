...
def makeFigure():
    ...
    plt.sca(ax2)
    ax2.text(.1, .6, 'Sort all the edges in ascendant order', 
...
def renderFrame(i, edges, nodes):
    if i > 0:
        ...
        plt.sca(ax2)
        ax2.text(.1, .6, 
            'Sort all the edges in ascendant order', 
            fontsize=15)
        ax2.text(.1, .5, 
...
'      If the edge does not make a cycle append it to the mst', 
        ...
        if (len(history) > 0):
            ...
            nx.draw_networkx_edges(G, pos, 
                edgelist=mst, width=4, edge_color='b' )
            plt.sca(ax2)
            ax2.set_xticks([])
            ax2.text(.1, .6, 
                'Sort all the edges in ascendant order', 
...
# Start animation
anim.append(animation.FuncAnimation(fig,
    renderFrame,fargs=[edges, nodes], frames=len(history)+2, 
    interval=1400))
